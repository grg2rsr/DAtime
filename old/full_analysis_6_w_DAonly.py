...
fr_opts = dict(sampling_period=5*pq.ms, kernel=ele.kernels.AlphaKernel(sigma=25*pq.ms))
...
f_samp = sp.linspace(-6,6,t_samp.shape[0])
M = sp.zeros((t_samp.shape[0],f_samp.shape[0]))
for i,t in enumerate(t_samp):
    M[i,:] = kdes[i].evaluate(f_samp)

plt.matshow(M.T,origin='bottom')
# make the heatmap
# verify by slicing
...
# build distributions from red stim
bin_size = 0.1* pq.s
# stim_ind = StimsDf.groupby('stim_id').get_group(k).index[0]
t_start = calc_stim_end(k, StimMap)
t_start = 0.5*pq.s
t_stop = t_start + 2*pq.s


# probability distribution

# get spiketrains
stim_times = StimsDf.groupby(['stim_id','opto']).get_group((k,'red'))['t'].values * pq.s

# for neuron 0
i = 0
St = SpikeTrains_sig[i]
kdes = []
for time in stim_times:
    spike_times = St.time_slice(time+t_start,time+t_stop).times - time
    kde = sp.stats.gaussian_kde(spike_times.magnitude)
    kdes.append(kde)

t_vec = sp.linspace(0.5,2.5,100)
M = sp.zeros((len(kdes),t_vec.shape[0]))
for i in range(len(kdes)):
    for j,t in enumerate(t_vec):
        M[i,j] = kdes[i].evaluate(t)


plt.matshow(M)
plt.gca().set_ylabel('trials')
...
# get the background KDEs
all_kdes  = []
for St in tqdm(SpikeTrains_sig):
    all_spiketimes = []
    for time in stim_times:
        spike_times = St.time_slice(time+t_start,time+t_stop).times - time
        all_spiketimes.append(spike_times.magnitude)
    all_spiketimes = sp.concatenate(all_spiketimes)
    kde = sp.stats.gaussian_kde(all_spiketimes)
    all_kdes.append(kde)
...
# get values
t = 0.5
t_vec = sp.linspace(0.5,2.5,150)
D = sp.zeros((len(all_kdes),t_vec.shape[0]))
for i,t in enumerate(tqdm(t_vec)):
    D[:,i] = [kde.evaluate(t)[0] for kde in all_kdes]

plt.matshow(D, origin='lower')
plt.gca().set_aspect('auto')

# likelihood for time t given sample ts
for td in t_vec: # decoded time

n = t_vec.shape[0]
L = sp.zeros((n,n))

for ts in t_vec:
    sample_resp = [kde.evaluate(ts)[0] for kde in all_kdes]
    # = the firing rates at a given timepoint
    for td in t_vec:
        # here: the likelihood of the firing rate coming from this timepoint 
