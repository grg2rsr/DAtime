...
for i in range(Rates_opto_.shape[0]):
    for j in range(Rates_opto_.shape[1]):
        Rates_opto_[i,j].t_start = -1*pq.s

# underscore denotes working copy
# Rates_ = copy(Rates)
...
label_stim_artifact(plt.gca(),stim_k,StimMap,0.25*pq.s,axis='x')
...
for ax in axes[0,:]:
    label_stim_artifact(ax,stim_k,StimMap,0.25*pq.s,axis='x')
    label_stim_artifact(ax,stim_k,StimMap,0.25*pq.s,axis='y')
...
# axes[1].plot(tt_dc,tt_decoded - tt_dc,'.',color='firebrick',alpha=0.8)
# axes[1].plot(tt_dc,tt_decoded_opto - tt_dc,'.',color='darkcyan',alpha=0.8)
# axes[1].plot(tt_dc,tt_dc  - tt_dc,':',alpha=0.5,color='k')
# axes[1].set_xlabel('real time (s)')
# axes[0].set_ylabel('dt')

label_stim_artifact(axes,stim_k,StimMap,0.25*pq.s,axis='x')
